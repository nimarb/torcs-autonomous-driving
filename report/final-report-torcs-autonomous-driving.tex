...
to simulate a car on a race track and collect relevant sensor data. The goal is to use multiple sensor cues to drive the car safely around the defined test tracks. Through the Robot Operating System (ROS) TODO: ADD CITATION the following sensors can be read from TORCS TODO: ADD ROS_TORCS CITATION: 
\begin{enumerate}
	\item range finder
...
To improve the controller's performance, it is possible to further increase the number of neurons. However, the processor that is used for simulation is already at its peak with the above mentioned neuron sizes. Furthermore, a detailed comparison between the infered and original output signals can be made. 
