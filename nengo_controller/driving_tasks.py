...
class Steer(nengo.Network):
    def __init__(self, name, input_ensemble, output_node, mode='default'):
        ...
        with self:
            steer_ensemble = nengo.Ensemble(n_neurons=4*n_neurons, dimensions=1)
...
class Accelerate(nengo.Network):
    def __init__(self, name, input_ensemble, output_node, mode='default'):
        ...
        with self:
            accel_ensemble = nengo.Ensemble(n_neurons=4*n_neurons, dimensions=1)
...
class Brake(nengo.Network):
    def __init__(self, name, input_ensemble, output_node, mode='default'):
        ...
        with self:
            brake_ensemble = nengo.Ensemble(n_neurons=4*n_neurons, dimensions=1)
